...
import kopo.poly.persistance.redis.IMoiveMapper;
import kopo.poly.persistance.redis.impl.MovieMapper;
...
public class MovieService implements IMovieService {
    ...
    private final IMoiveMapper moiveMapper; // RedisDB 저장할 Mapper
    ...
    private int collectMovie(String redisKey) throws Exception {
        ...
        Iterator<Element> score = element.select("span.percent").iterator();    // 접수
        ...
        while (movie_rank.hasNext()) {
            MovieDTO pDTO = MovieDTO.builder()
                    ...
                    .openDay(CmmUtil.nvl(open_day.next().text()).trim().substring(0, 10)) // 개봉일
